...
function Out-Replies {
	...
	for ($i=0; $i -lt $(if ($CurrentPage -ne $CommentPageCount) {$CommentsPerPage} else {$Comments|jq "length"}); ++$i) {
		...
		if (!$CachedDirState) { 
			New-Item -ItemType Directory -Force -Path (Join-Path $CommentDir "page${CurrentPage}_replies")|Out-Null #putting this in the for loop avoids the creation of empty folders
			$CachedDirState=$True
		}
		...
		for ($j=1; $j -le $(if ($ReplyPageCount -gt $ReplyPageLimit) {$ReplyPageLimit} else {$ReplyPageCount}); ++$j) {
			Get-WebJson -pn $j -root $rpid|Out-File (Join-Path $CommentDir "page${CurrentPage}_replies" "${Rpid}_page${j}.json")
		}
	}
...
$CommentDir=Join-Path $Path "comments"
...
if ($SaveMetadata) {
	...
	}|ConvertTo-Json|Out-File (Join-Path $Path "comments" "metadata.json")
}

Write-Host "Saving page $CurrentPage..."

$Json|Out-File (Join-Path $CommentDir "page${currentPage}.json")
$Json|Out-Replies 

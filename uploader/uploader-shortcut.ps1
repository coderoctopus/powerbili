...
param (
	...
	[switch]$NoStats,
	[switch]$NoCommunity #does nothing if both switches are on
)

$UploaderPath = Join-Path $Path $Uid #todo: change to username maybe?
New-Item -ItemType Directory -Force -Path $UploaderPath|Out-Null

if (!$NoStats) {
	Write-Host "Saving uploader statistics..."
	(Invoke-WebRequest "https://api.bilibili.com/x/space/upstat?mid=$Uid&jsonp=jsonp").Content|Out-File -LiteralPath (Join-Path $UploaderPath "stats.json") #why on earth can't -LiteralPath simply be the default option?

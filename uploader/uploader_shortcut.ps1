...
param (
	...
	[ValidateScript({Test-Path $_}, ErrorMessage="The specified directory does not exist.")][string]$Path="." #dir for all uploaders
)
...
(Invoke-WebRequest "https://api.bilibili.com/x/space/upstat?mid=$Uid&jsonp=jsonp").Content|Out-File (Join-Path $UploaderPath "stats.json")
